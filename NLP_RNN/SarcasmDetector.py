...
trainingPaddedSeqs = pad_sequences(trainingSeqs, padding='post')
...
tokenizerTest = tf.keras.preprocessing.text.Tokenizer(oov_token="<OOV>", num_words=10000)
tokenizerTest.fit_on_texts(test_sentences)
testSeqs = tokenizerTest.texts_to_sequences(test_sentences)
testPaddedSeqs = pad_sequences(testSeqs, padding='post')
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(24, tf.nn.relu),
...
model.fit(trainingPaddedSeqs, training_labels, epochs=20, validation_data=(testPaddedSeqs, test_labels))
...
padded = pad_sequences(evalSeq, padding='post')
